...
const replacement = String.raw`function VenuePage({ user, setUser, saved, toggleSave }) {
  ...
  const checkAvailability = async () => {
    ...
    try {
      ...
      else if (result.status === 'held') setMessage('This date is currently on hold. Please choose another date.');
      else setMessage('This date is already booked. Please choose another date.');
    } catch (e) {
  ...
  const submit = async (action = bookingAction) => {
    ...
    try {
      ...
      if (action === 'hold' && !current.canHold) return setMessage('This date cannot be held because it is already booked or held.');
      ...
      setMessage(result.message || (action === 'book' ? 'Venue booked successfully.' : 'Your venue hold has been created.'));
      setBookingAction(action);
    } catch (e) {
  ...
  return <main className="venue-detail">
    ...
    <div className="detail-body">
      ...
      <div className="detail-section"><h2>Good to know</h2><p>Check live availability first, then choose either Instant Book or a refundable date hold when the selected date is available.</p></div>
...
function BookingSheet({venue,date,setDate,event,setEvent,action,availability,checkingAvailability,submit,message,close}) {
  const actionLabel = action === 'book' ? 'Book venue' : action === 'hold' ? 'Hold this date' : 'Check availability';
  ...
  return <div className="sheet-backdrop" onClick={close}><div className="booking-sheet" onClick={e => e.stopPropagation()}>
    ...
    {availability && <div className={\`notice \${availability.status === 'available' ? '' : 'error'}\`}><b>{availability.status === 'available' ? 'Available' : availability.status === 'held' ? 'On hold' : 'Booked'}</b> · {date}</div>}
    <div className="hold-selected-venue"><span>Selected venue</span><b>{venue.name}</b><small>{venue.city} · {money(venue.price)}</small></div>
    <div className="hold-summary"><div><span>Venue price</span><b>{money(venue.price)}</b></div><div><span>Refundable hold fee</span><b>{money(holdFee(venue.price))}</b></div><small>Hold fee is 10% of venue price, with a ₹15,000 minimum and ₹75,000 maximum.</small></div>
    ...
    <div className="booking-action-grid">
      ...
      <button className="outline-button wide" onClick={() => submit('book')} disabled={!!availability && !availability.canBook}>{action === 'book' ? actionLabel : 'Book venue'}</button>
    ...
    {available && <small className="booking-hint">This date is available. You can now book it or place a refundable hold.</small>}
  </div></div>;
}
...
console.log('Venue Search frontend patch applied.');
