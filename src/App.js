...
function VenuePage({ user, setUser, saved, toggleSave }) {
  const { id } = useParams();
  const navigate = useNavigate();
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!payload.venueId || !payload.venueName) {
        return setMessage("This venue could not be identified. Please reopen the venue page.");
      }
      if (!user) {
      ...
      const result = await api.createHold(token, payload);
      setMessage(result.message || "Your venue hold has been created.");
    } catch(e) { setMessage(e.message || "Unable to save the date hold."); }
  };
  ...
  return <main className="venue-detail">
    ...
    <div className="detail-body">
      ...
      <div className="detail-stats"><Stat icon={Users} value={`${venue.capacity}`} label="Guests"/><Stat icon={WalletCards} value={money(venue.price)} label="Starting from"/><Stat icon={Clock3} value="72 hrs" label="Hold window"/></div>
      <div className="detail-section"><h2>Why couples choose it</h2><div className="amenity-grid">{["Verified capacity","Transparent pricing","Real venue photos","Date availability","Wedding support","Secure hold"].map(x=><div key={x}><Check size={15}/>{x}</div>)}</div></div>
      <div className="detail-section"><h2>Good to know</h2><p>Venue Search is designed to reduce the manual coordination around venue discovery. Main venues use an Instant Hold workflow, while smaller event spaces can use Instant Book where enabled.</p></div>
      <div className="sticky-cta"><div><small>Starting from</small><b>{money(venue.price)}</b></div><button className="primary-button" onClick={()=>setShowBooking(true)}>{venue.mode === "Instant Book" ? "Book venue" : "Hold this date"}</button></div>
    </div>
    {showBooking && <BookingSheet venue={venue} date={date} setDate={setDate} event={event} setEvent={setEvent} submit={submit} message={message} close={()=>setShowBooking(false)}/>}
  </main>;
}

function Stat({icon:Icon,value,label}) { return <div><Icon size={18}/><b>{value}</b><span>{label}</span></div>; }

function BookingSheet({venue,date,setDate,event,setEvent,submit,message,close}) {
  return <div className="sheet-backdrop" onClick={close}><div className="booking-sheet" onClick={e=>e.stopPropagation()}>
    <div className="sheet-head"><div><span className="eyebrow">SECURE YOUR DATE</span><h2>{venue.name}</h2></div><button className="icon-button" onClick={close}><X/></button></div>
    <label className="input-label">Wedding date<input type="date" value={date} onChange={e=>setDate(e.target.value)}/></label>
    ...
    <div className="hold-selected-venue"><span>Selected venue</span><b>{venue.name}</b><small>{venue.city} · {money(venue.price)}</small></div><div className="hold-summary"><div><span>Venue price</span><b>{money(venue.price)}</b></div><div><span>Refundable hold fee</span><b>{money(holdFee(venue.price))}</b></div><small>Hold fee is 10% of venue price, with a ₹15,000 minimum and ₹75,000 maximum.</small></div>
    {message && <div className="notice">{message}</div>}
    <button className="primary-button wide" onClick={submit}>Continue securely <ArrowRight size={16}/></button>
  </div></div>;
}
